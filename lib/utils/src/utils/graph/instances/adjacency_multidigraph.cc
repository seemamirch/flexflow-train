...
AdjacencyMultiDiGraph::AdjacencyMultiDiGraph(
    ...
    std::map<Node, std::map<Node, std::set<MultiDiEdge>>> const &adjacency,
...
Node AdjacencyMultiDiGraph::add_node() {
  ...
  std::set<Node> all_nodes = set_union(keys(this->adjacency), {new_node});
  this->adjacency[new_node] = generate_map(
      all_nodes, [](Node const &) { return std::set<MultiDiEdge>{}; });
...
void AdjacencyMultiDiGraph::remove_node(Node const &n) {
  ...
  std::set<MultiDiEdge> outgoing = set_union(values(this->adjacency.at(n)));
...
std::set<Node> AdjacencyMultiDiGraph::query_nodes(NodeQuery const &q) const {
...
void AdjacencyMultiDiGraph::inplace_materialize_from(
  ...
  this->adjacency = generate_map(nodes, [&](Node const &) {
    return generate_map(nodes,
                        [&](Node const &) { return std::set<MultiDiEdge>{}; });
  });
