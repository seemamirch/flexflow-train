...
bidict<L, R> bidict_from_unstructured_relation(
  ...
  {
    std::multiset<L> l_values = transform(
      multiset_of(relation),
      [](std::pair<L, R> const &p) -> L {
        return p.first;
      }
    );
    ...
    std::map<L, positive_int> duplicated_element_counts =
      filter_values(l_value_counts,
                    [](positive_int num_occurences) -> bool {
                      return num_occurences > 1;
                    });

    ASSERT(duplicated_element_counts.empty(),
           duplicated_element_counts);
  }

  {
    std::multiset<R> r_values = transform(
      multiset_of(relation),
      [](std::pair<L, R> const &p) -> R {
        return p.second;
      }
    );
