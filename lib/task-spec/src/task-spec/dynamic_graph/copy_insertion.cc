...
void require_invocation_is_ready_for_copy_insertion(DynamicNodeInvocation const &i) {
  auto require_slot_is_ready_for_copy_insertion = [](DynamicTensorSlot const &) {
    return;
  };
...
void require_graph_is_ready_for_copy_insertion(DynamicOpenDataflowGraph const &g) {
  require_full_dynamic_graph_satisfies(g, require_invocation_is_ready_for_copy_insertion);
}
...
void require_invocation_is_fully_copy_inserted(DynamicNodeInvocation const &i) {
  auto require_node_is_copy_inserted = [](DynamicNodeAttrs const &) {
    return;
  };
...
void require_graph_is_fully_copy_inserted(DynamicOpenDataflowGraph const &g) {
  require_full_dynamic_graph_satisfies(g, require_invocation_is_fully_copy_inserted);
}
...
DynamicNodeInvocation make_copy_invocation(DynamicValueCopyInfo const &copy_info) {
  DynamicNodeInvocation result = DynamicNodeInvocation{
      /*inputs=*/{
          {
              DynamicTensorSlot{
              ...
              decide_dynamic_value_attrs_mapping(copy_info.value_attrs, copy_info.src_mapping),
          },
      },
      /*node_attrs=*/
      DynamicNodeAttrs{
      ...
      {
          {
              ...
              decide_dynamic_value_attrs_mapping(copy_info.value_attrs, copy_info.dst_mapping),
          },
      },
  };
...
std::set<DynamicValueCopyInfo> copies_for_value(
    ...
    std::map<InternalDynamicSlotSite, ParallelTensorMapping> const &all_mappings) {
  ...
  return src_site.visit<std::set<DynamicValueCopyInfo>>(overload {
    ...
    [&](InternalDynamicSlotSite const &s) -> std::set<DynamicValueCopyInfo> {
      ...
      std::map<InternalDynamicSlotSite, ParallelTensorMapping> sink_site_mappings =
        restrict_keys(all_mappings, dst_sites);
...
std::set<DynamicValueCopyInfo> copies_for_internal_value(
    ...
    std::map<InternalDynamicSlotSite, ParallelTensorMapping> const &sink_site_mappings) {
    ...
    resolve_tensor_mappings(DynamicOpenDataflowGraph const &g)
{
  ...
  std::map<InternalDynamicSlotSite, ParallelTensorMapping> resolved_from_node_mappings =
    resolve_partial_tensor_mappings_from_node_mappings(g);

  std::map<InternalDynamicSlotSite, ParallelTensorMapping> resolved_from_adjacent_values =
    resolve_missing_tensor_mappings_from_adjacent_values(g, resolved_from_node_mappings);

  std::map<InternalDynamicSlotSite, ParallelTensorMapping> result =
    binary_merge_disjoint_maps(resolved_from_node_mappings, resolved_from_adjacent_values);

  {
    std::set<InternalDynamicSlotSite> all_internal_slot_sites =
      get_internal_dynamic_slot_sites(g);
    std::set<InternalDynamicSlotSite> resolved_slot_sites =
      keys(result);
...
std::map<InternalDynamicSlotSite, ParallelTensorMapping>
  ...
  auto slots_to_map_for_replicate = [](dynamic_invocation_id_t const &invocation_id,
    ...
    -> std::set<InternalDynamicSlotSite>
  {
    ...
    std::set<InternalDynamicSlotSite> slot_sites = 
      ...
      ? get_incoming_dynamic_slot_sites_for_invocation(invocation_id, invocation)
      : get_output_dynamic_slot_sites_for_invocation(invocation_id, invocation);
  ...
  auto get_mappings_for_invocation = [&](DynamicNodeInvocation const &invocation)
      -> std::map<InternalDynamicSlotSite, ParallelTensorMapping> {

    TrainingOpType op_type = dynamic_node_invocation_get_op_type(invocation);
    dynamic_invocation_id_t invocation_id = dynamic_graph_get_id_for_invocation(g, invocation);
        ...
        [&](InternalDynamicSlotSite const &s) -> ParallelTensorMapping {
          return ParallelTensorMapping{
              dynamic_node_mapping_bindings_for_slot_name(
                  assert_unwrap(invocation.node_attrs.mapping), s.slot_name.slot_name),
          };
        });
  };
...
std::map<InternalDynamicSlotSite, ParallelTensorMapping>
    resolve_missing_tensor_mappings_from_adjacent_values(
      DynamicOpenDataflowGraph const &g,
      std::map<InternalDynamicSlotSite, ParallelTensorMapping> const &resolved_mappings)
{
  require_graph_is_ready_for_copy_insertion(g);
  ...
  auto get_mapping_for_slot_site_from_adjacent_values =
    ...
    DynamicNodeInvocation invocation = dynamic_graph_get_invocation_for_id(g, slot_site.invocation_id);
    ...
    if (op_type == replicate_op_type && task_type == DynamicTaskType::BWD) {
      ...
      return resolved_mappings.at(slot_site_sink);
        ...
        op_type == replicate_op_type 
        && (task_type == std::nullopt || task_type == DynamicTaskType::FWD)
    ) {
      ASSERT(slot_site.direction == TensorDirection::INCOMING);

      InternalDynamicSlotSite slot_site_src =
        dynamic_graph_find_source_of_slot_site(g, slot_site).require_internal();
...
std::set<DynamicValueCopyInfo> infer_all_copies_in_graph(DynamicOpenDataflowGraph const &g) 
{
  std::map<InternalDynamicSlotSite, ParallelTensorMapping>
      fully_resolved_tensor_mappings =
          resolve_tensor_mappings(g);

  std::set<DynamicValueCopyInfo> all_copies =
      flatmap(set_of(get_dynamic_values(g)),
              [&](DynamicValueAttrs const &v)
                ...
                std::set<InternalDynamicSlotSite> sinks = dynamic_graph_find_sinks_of_value(g, v);

                return copies_for_value(v, src_site, sinks, fully_resolved_tensor_mappings);
              });
...
DynamicOpenDataflowGraph
  ...
  std::set<DynamicNodeInvocation> all_copy_invocations = transform(all_copies, make_copy_invocation);
  ...
  std::set<DynamicNodeInvocation> mapped_invocations = transform(
      ...
      [&](DynamicNodeInvocation const &i) -> DynamicNodeInvocation {
        ...
        return apply_mappings_for_invocation(id, i, fully_resolved_tensor_mappings);
      });
