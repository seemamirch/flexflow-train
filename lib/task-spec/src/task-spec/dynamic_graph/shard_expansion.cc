...
void require_invocation_is_fully_shard_expanded(DynamicNodeInvocation const &i) {
...
void require_graph_is_fully_shard_expanded(DynamicOpenDataflowGraph const &g) {
  return require_full_dynamic_graph_satisfies(g, require_invocation_is_fully_shard_expanded);
}
...
void require_invocation_is_ready_for_shard_expansion(DynamicNodeInvocation const &i) {
  auto require_slot_is_ready_for_shard_expansion = [](DynamicTensorSlot const &) {
    return;
  };
...
void require_graph_is_ready_for_shard_expansion(DynamicOpenDataflowGraph const &g) {
  require_full_dynamic_graph_satisfies(g, require_invocation_is_ready_for_shard_expansion);
}
...
static DynamicNodeInvocationShardingInfo invocation_sharding_info_for_binding(
  ...
  auto shard_expand_value_attrs =
    ...
    return DynamicValueAttrsShardingInfo{
      ...
      /*mapping=*/pt_mapping_get_device_for_coord(assert_unwrap(v.mapping), parallel_tensor_coord),
    };
  };
  ...
  DynamicNodeInvocationShardingInfo result = DynamicNodeInvocationShardingInfo{
      ...
      /*value_sharding=*/binary_relation_transform_right2(
  ...
  {
    std::set<DynamicTensorSlot> invocation_slots = set_union(
      keys(i.inputs), keys(i.outputs));
...
static bidict<ParallelTensorSpaceCoordinate, global_device_id_t>
  ...
  return bidict_filter_keys(mapping, [&](ParallelTensorSpaceCoordinate const &p) {
...
static std::set<DynamicNodeInvocationShardingInfo>
  ...
  return transform(
      ...
      [&](ParallelTensorSpaceCoordinate const &p) -> DynamicNodeInvocationShardingInfo {
        ...
        global_device_id_t device_id = pt_mapping_get_device_for_coord(input_mapping, p);
...
static std::set<DynamicNodeInvocationShardingInfo>
  ...
  std::set<ParallelTensorSpaceCoordinate> input_tensor_shards = pt_mapping_get_coord_set(input_value_mapping);
  ...
  {
    bidict<global_device_id_t, OperatorAtomicTaskShardBinding> dependent_on_input_tensor_shard
    ...
    -> DynamicNodeInvocationShardingInfo
  {
    ...
    std::map<global_device_id_t, DynamicValueAttrsShardingInfo> output_sharding_infos =
      generate_map(task_shard_device_ids.unwrap_as_set(),
                   [&](global_device_id_t const &device_id)
                     -> DynamicValueAttrsShardingInfo
                   {
                     ParallelTensorSpaceCoordinate pc
                       = pt_mapping_get_coord_for_device(output_value_mapping, device_id);
...
static std::set<DynamicNodeInvocationShardingInfo>
  ...
  ParallelTensorMapping output_grad_value_mapping = assert_unwrap(output_grad.mapping);
  ParallelTensorMapping input_grad_value_mapping = assert_unwrap(input_grad.mapping);
  ...
  auto get_task_shard_device_ids_for_input_grad_tensor_shard
    = [&](ParallelTensorSpaceCoordinate const &input_grad_tensor_shard)
        -> nonempty_set<global_device_id_t>
  {
    bidict<global_device_id_t, OperatorAtomicTaskShardBinding> produce_input_grad_tensor_shard
      = bidict_filter_values(
          dynamic_node_mapping_get_shard_bindings(node_mapping),
          [&](OperatorAtomicTaskShardBinding const &b) -> bool {
    ...
    -> DynamicNodeInvocationShardingInfo
  {
    ...
    std::map<global_device_id_t, DynamicValueAttrsShardingInfo> output_grad_sharding_infos =
      generate_map(task_shard_device_ids.unwrap_as_set(),
                   [&](global_device_id_t const &device_id)
                     -> DynamicValueAttrsShardingInfo
                   {
                     ParallelTensorSpaceCoordinate pc
                      = pt_mapping_get_coord_for_device(output_grad_value_mapping, device_id);
    ...
    DynamicValueAttrsShardingInfo input_grad_sharding_info = DynamicValueAttrsShardingInfo{
      /*shard_coord=*/c,
      /*mapping=*/pt_mapping_get_device_for_coord(input_grad_value_mapping, c),
    };
  ...
  DynamicValueAttrsShardingInfo const &value_sharding_info)
{
  ...
  if (result.mapping.has_value()) {
    ...
    global_device_id_t from_mapping =
      pt_mapping_get_device_for_coord(value_mapping, value_sharding_info.shard_coord);
  ...
  DynamicNodeInvocationShardingInfo const &invocation_sharding_info)
{
  ...
  {
    std::set<DynamicTensorSlot> invocation_slots = set_union(
      keys(invocation.inputs), keys(invocation.outputs));
  ...
  std::set<DynamicTensorSlot> shard_labelled =
    filtrans(invocation_sharding_info.value_sharding.left_values(),
      ...
      -> std::map<DynamicTensorSlot, DynamicValueAttrs>
  {
    ...
    if (contains(shard_labelled, slot)) {
      BinaryRelation<DynamicTensorSlot, DynamicValueAttrsShardingInfo> 
        for_slot = filter_binary_relation(invocation_sharding_info.value_sharding,
                                        [&](DynamicTensorSlot const &s, 
      ...
      std::set<std::pair<DynamicTensorSlot, DynamicValueAttrs>> result = transform(
        for_slot.unwrap_as_set(),
        [&](std::pair<DynamicTensorSlot, DynamicValueAttrsShardingInfo> const &p) 
          -> std::pair<DynamicTensorSlot, DynamicValueAttrs>
        {
          return {p.first, apply_dynamic_value_attrs_sharding_info(value_attrs, p.second)};
        });
      ...
      return {
        {
          slot,
          apply_dynamic_value_attrs_sharding_info(value_attrs, sharding_info),
        },
      };
    }
  };
  ...
  generate_shard_expansion_for_invocation(DynamicNodeInvocation const &i)
{
  ...
  std::set<DynamicNodeInvocationShardingInfo> result = [&]() {
    if (i.node_attrs.op_attrs.value().is_copy()) {
    ...
    } else if (training_op_attrs_has_op_type(i.node_attrs.op_attrs.value(), OperatorType::REPLICATE)) {
      ...
      return transform(shard_machine_coords,
                       [&](global_device_id_t const &device_id) -> DynamicNodeInvocationShardingInfo {
                         OperatorAtomicTaskShardBinding slot_bindings =
                           dynamic_node_mapping_get_shard_binding_for_device(mapping, device_id);

                         return invocation_sharding_info_for_binding(i, device_id, slot_bindings);
                       });
    }
  }();
