...
void require_invocation_is_fully_pass_expanded(DynamicNodeInvocation const &invocation) {
  auto require_slot_is_pass_expanded = [&](DynamicTensorSlot const &s) {
    if (dynamic_node_invocation_get_op_type(invocation) == TrainingOpType{TrainingOnlyOpType::COPY}) {
      return;
    }
...
void require_invocation_is_ready_for_pass_expansion(DynamicNodeInvocation const &invocation) {
  require_invocation_fully_satisfies(
      invocation, 
...
void require_graph_is_fully_pass_expanded(DynamicOpenDataflowGraph const &g) {
  require_full_dynamic_graph_satisfies(
      g, 
      require_invocation_is_fully_pass_expanded);
}

void require_graph_is_ready_for_pass_expansion(DynamicOpenDataflowGraph const &g) {
  require_full_dynamic_graph_satisfies(
      g, 
      require_invocation_is_ready_for_pass_expansion);
}
    ...
    DynamicValueAttrs const &val)
{
  auto get_values_immediately_needed_to_compute_gradients_of_needed =
    [&](std::set<DynamicValueAttrs> const &needed)
      -> std::set<DynamicValueAttrs>
    {
      std::set<DynamicValueAttrs> additional 
        = flatmap(needed,
                     [&](DynamicValueAttrs const &v) -> std::set<DynamicValueAttrs> {
                       std::set<InternalDynamicSlotSite> sinks =
                         dynamic_graph_find_sinks_of_value(g, v);

                       return flatmap(
                         sinks,
                         [&](InternalDynamicSlotSite const &sink) -> std::set<DynamicValueAttrs> {
    ...
    DynamicOpenDataflowGraph const &g)
{
  auto value_is_fundamentally_required =
    [&](DynamicValueAttrs const &v) -> bool {
      ...
      TrainingOpType op_type = dynamic_node_invocation_get_op_type(source_invocation);
  ...
  std::set<DynamicValueAttrs> required_values =
    flatmap(fundamentally_required_values,
            [&](DynamicValueAttrs const &fundamentally_required_value) -> std::set<DynamicValueAttrs> {
  ...
  auto get_sink_invocations_for_value = 
    [&](DynamicValueAttrs const &v) -> std::set<dynamic_invocation_id_t> {
      return 
        transform(
          dynamic_graph_find_sinks_of_value(g, v),
          [&](InternalDynamicSlotSite const &sink_site) -> dynamic_invocation_id_t {
...
DynamicNodeInvocation perform_bwd_pass_expansion_for_invocation(
    ...
    -> DynamicNodeInvocation
  {
    ...
    } else if (training_op_attrs_has_op_type(op_attrs, OperatorType::REPLICATE)) {
      return DynamicNodeInvocation{
          /*inputs=*/{
