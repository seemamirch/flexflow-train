...
static DynamicOpenDataflowGraph mk_single_input_node_graph(MachineSpaceCoordinate const &input_device)
{
  ...
  auto mk_node_mapping = [](MappedOperatorTaskGroup const &op_task_group) -> DynamicNodeMapping {
  ...
  auto mk_pt_coord = [](nonnegative_int idx) -> ParallelTensorSpaceCoordinate {
    return ParallelTensorSpaceCoordinate{
      ...
      /*shared_components=*/FFOrdered<nonnegative_int>{
        0_n,
        0_n,
      },
    };
  ...
  MappedOperatorTaskGroup input_node_mapping =
...
static DynamicOpenDataflowGraph mk_single_input_into_relu_graph(MachineSpaceCoordinate const &input_device,
                                                                MachineSpaceCoordinate const &relu1_device)
{
  ...
  MappedOperatorTaskGroup relu1_node_mapping =
    MappedOperatorTaskGroup{
      bidict<MachineSpaceCoordinate, OperatorAtomicTaskShardBinding>{
        {
          relu1_device,
...
static ExampleGraphTestCase mk_example_replicate_graph(
  ...
  MachineSpaceCoordinate const &relu2_device2)
{
  ...
  MappedOperatorTaskGroup replicate_node_mapping =
    MappedOperatorTaskGroup{
      bidict<MachineSpaceCoordinate, OperatorAtomicTaskShardBinding>{
        {
          replicate_device1,
  ...
  MappedOperatorTaskGroup relu2_node_mapping =
    MappedOperatorTaskGroup{
      bidict<MachineSpaceCoordinate, OperatorAtomicTaskShardBinding>{
        {
          relu2_device1,
  ...
  DynamicOpenDataflowGraph g
    = dynamic_open_dataflow_graph_from_invocation_set(
        {input_invocation, relu1_invocation, replicate_invocation, relu2_invocation});
...
TEST_SUITE(FF_TEST_SUITE) {
    ...
    SUBCASE("dynamic graph is not pass expanded") {
      auto mk_correct_mappings =
          ...
          -> std::map<InternalDynamicSlotSite, ParallelTensorMapping>
        {
          ...
          auto mk_inp_slot = [](dynamic_invocation_id_t invocation_id) -> InternalDynamicSlotSite {
          ...
          auto mk_out_slot = [](dynamic_invocation_id_t invocation_id) -> InternalDynamicSlotSite {
            return InternalDynamicSlotSite{
              /*invocation_id=*/invocation_id,
          ...
          auto mk_single_shard_mapping = [&](MachineSpaceCoordinate const &mc) -> ParallelTensorMapping {
          ...
          auto mk_two_shard_mapping = [&](MachineSpaceCoordinate const &mc1,
      ...
      SUBCASE("replicate input matches tensor source") {
        ExampleGraphTestCase tc
          = mk_example_replicate_graph(
            /*input_device=*/mc1,
            /*relu1_device=*/mc1,
  ...
  TEST_CASE("copies_for_value") {
    ...
    auto mk_slot_site = [](nonnegative_int idx) -> InternalDynamicSlotSite {
      return InternalDynamicSlotSite{
        ...
        /*slot_name=*/DynamicTensorSlot{
          ...
          /*slot_tensor_role=*/DynamicTensorRole{FwbTensorType::FORWARD}, // could be any role
          /*task_shard=*/std::nullopt,
        },
      };
    };
    ...
    SUBCASE("if src site is external, no copies no matter what") {
      ...
      std::set<DynamicValueCopyInfo> result =
        copies_for_value(
    ...
    SUBCASE("if src mapping matches one dst mapping, still issue copies for the rest") {
      ParallelTensorMapping mapping1 = ParallelTensorMapping{
        bidict<ParallelTensorSpaceCoordinate, global_device_id_t>{
          {mk_pt_coord(0_n), mk_device(0_n)},
        },
  ...
  TEST_CASE("perform_copy_insertion") {
    ...
    SUBCASE("standard operator") {
      ...
      SUBCASE("inserts copy when necessary") {
        ...
        DynamicNodeInvocation inv1 = DynamicNodeInvocation{
            ...
            /*node_attrs=*/
            mk_node_attrs(
                mk_pcg_layer_guid(1), mapping1, relu_attrs),
            /*outputs=*/
            {
                {
                    mk_slot(TensorSlotName::OUTPUT),
        ...
        DynamicNodeInvocation inv2 = DynamicNodeInvocation{
            ...
            /*node_attrs=*/
            mk_node_attrs(
                mk_pcg_layer_guid(2), mapping2, relu_attrs),
            /*outputs=*/
            {
                {
                    mk_slot(TensorSlotName::OUTPUT),
    ...
    SUBCASE("replicate operator") {
      ...
      DynamicOpenDataflowGraph correct = [&] {
        auto map_input_value = [](DynamicNodeInvocation const &invocation,
          ...
          result.inputs = {
            {
              ...
              decide_dynamic_value_attrs_mapping(
                require_only_key(invocation.inputs, input_slot),
                mapping),
            },
          };
          return result;
        ...
        auto map_output_value = [](DynamicNodeInvocation const &invocation,
                                   ParallelTensorMapping const &mapping)
          -> DynamicNodeInvocation
        {
          DynamicNodeInvocation result = invocation;
        ...
        auto map_input_and_output_values = [&](DynamicNodeInvocation const &invocation,
        ...
        DynamicNodeInvocation input_invocation = dynamic_graph_get_invocation_for_id(tc.g, tc.input_op_id);
        DynamicNodeInvocation relu1_invocation = dynamic_graph_get_invocation_for_id(tc.g, tc.relu1_op_id);
        DynamicNodeInvocation replicate_invocation = dynamic_graph_get_invocation_for_id(tc.g, tc.replicate_op_id);
        DynamicNodeInvocation relu2_invocation = dynamic_graph_get_invocation_for_id(tc.g, tc.relu2_op_id);
        ...
        DynamicNodeInvocation input_to_relu1_copy = DynamicNodeInvocation{
          /*inputs=*/{
            {
              mk_slot(TensorSlotName::INPUT),
              require_only_key(value_mapped_input_invocation.outputs, mk_slot(TensorSlotName::OUTPUT)),
            },
          },
          ...
          /*outputs=*/{
            {
              mk_slot(TensorSlotName::OUTPUT),
              require_only_key(value_mapped_relu1_invocation.inputs, mk_slot(TensorSlotName::INPUT)),
            },
          },
        };
        ...
        DynamicNodeInvocation replicate_to_relu2_copy = DynamicNodeInvocation{
          /*inputs=*/{
            {
              mk_slot(TensorSlotName::INPUT),
              require_only_key(value_mapped_replicate_invocation.outputs, mk_slot(TensorSlotName::OUTPUT)),
            },
          },
          /*node_attrs=*/DynamicNodeAttrs{
            /*task_type=*/std::nullopt,
          ...
          /*outputs=*/{
            {
              mk_slot(TensorSlotName::OUTPUT),
              require_only_key(value_mapped_relu2_invocation.inputs, mk_slot(TensorSlotName::INPUT)),
            },
          },
        };
      ...
      nlohmann::json result_json = dynamic_open_dataflow_graph_to_serializable(result);
      nlohmann::json correct_json = dynamic_open_dataflow_graph_to_serializable(correct);
    ...
    SUBCASE("copy insertion commutes with pass expansion") {
      ...
      SUBCASE("graph is single input node") {
        ...
        nlohmann::json pass_expansion_before_copy_insertion_json
          = dynamic_open_dataflow_graph_to_serializable(pass_expansion_before_copy_insertion);
        nlohmann::json copy_insertion_before_pass_expansion_json
          = dynamic_open_dataflow_graph_to_serializable(copy_insertion_before_pass_expansion);

        CHECK_MESSAGE(
            pass_expansion_before_copy_insertion == copy_insertion_before_pass_expansion,
            ...
            check_kv("pass_expansion_before_copy_insertion\n",
                     dynamic_open_dataflow_graph_as_dot(pass_expansion_before_copy_insertion)),
            check_kv("copy_insertion_before_pass_expansion\n",
                     dynamic_open_dataflow_graph_as_dot(copy_insertion_before_pass_expansion)));
      }

      SUBCASE("graph is single input node followed by relu") {
        SUBCASE("operations are mapped to the same device") {
          DynamicOpenDataflowGraph g = mk_single_input_into_relu_graph(mc1, mc1);
        ...
        SUBCASE("operations are mapped to different devices") {
          DynamicOpenDataflowGraph g = mk_single_input_into_relu_graph(mc1, mc2);
